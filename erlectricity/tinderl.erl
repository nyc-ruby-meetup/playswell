-module(tinderl).
-export([start/3, stop/0, speak/1, paste/1]).

start(Domain, Token, Room) ->
  spawn(fun() ->
    register(tinderl, self()),
    process_flag(trap_exit, true),
    % Cmd = lists:flatten(io_lib:format("ruby ./tinderl.rb ~s ~s ~s ~s", [Domain, Login, Password, Room])),
	Cmd = lists:flatten(io_lib:format("ruby ./tinderl.rb ~s ~s ~s", [Domain, Token, Room])),
